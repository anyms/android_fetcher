...
class PausedFragment : Fragment(), FragmentListener, IdmListener {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val v = inflater.inflate(R.layout.fragment_paused, container, false)
    ...
    override fun onShow() {
        ...
        (activity as? MainActivity)?.apply {
            if (isBound) {
                idmService?.idmListener = this@PausedFragment
            } else {
                idmListener = this@PausedFragment
            }
        }
    }
    ...
    private fun updateRecyclerView() {
        snapshots.clear()
        thread {
            val snaps = idmDatabase.idmDao().getSnapshots(IdmSnapshot.STATUS_PAUSED)
